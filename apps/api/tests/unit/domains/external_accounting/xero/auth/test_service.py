...
class TestXeroService:
    ...
    def xero_service(self, mock_prisma: Mock, mock_settings: Mock) -> XeroService:
        ...
        with patch(
            "src.domains.external_accounting.xero.auth.service.settings", mock_settings
        ):
    ...
    async def test_start_connection_success(
        ...
        # Act
        with patch(
            "src.domains.external_accounting.xero.auth.service.jwt.encode"
        ) as mock_jwt_encode:
    ...
    def test_validate_state_token_success(
        ...
        # Act
        with patch(
            "src.domains.external_accounting.xero.auth.service.jwt.decode"
        ) as mock_jwt_decode:
